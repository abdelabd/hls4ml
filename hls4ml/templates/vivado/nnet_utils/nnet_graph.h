...
namespace nnet {
			...
			typename CONFIG_T::dense_config4::bias_t    core_edge_b3[CONFIG_T::dense_config4::n_out])
  {
    ...
    #pragma HLS ARRAY_PARTITION variable=edge_update_aggr complete dim=0
    if((CONFIG_T::aggr == aggr_sum)||(CONFIG_T::aggr==aggr_mean)){ //if sum or mean aggregation, initialize aggregate as zeros
      for(int i=0; i < CONFIG_T::n_node; i++){
    ...
    if(CONFIG_T::aggr == aggr_max){ //note: the edge_update_aggr array has been initialized but IS NOT ZEROS
      for(int i=0; i < CONFIG_T::n_edge; i++){
        ...
        if(num_edge_per_node[r] < 1){ //if node has zero edges, node should have edge_aggr=<0,...,0>
          for(int j=0; j<CONFIG_T::out_dim; j++){
            #pragma HLS UNROLL
            edge_update_aggr[r][j] = 0;
          }
        }

        else if(num_edge_per_node[r] == 1){ //if it only has one edge, there's nothing to aggregate with
          for(int j=0; j<CONFIG_T::out_dim; j++){
            #pragma HLS UNROLL
            edge_update_aggr[r][j] = edge_update[i][j];
          }
        ...
        else{ //if it has more than one edge, perform max-aggregation as usual
          for(int j=0; j<CONFIG_T::out_dim; j++){
            #pragma HLS UNROLL
